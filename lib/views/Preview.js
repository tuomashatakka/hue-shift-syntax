...
export default class PreviewView extends Component {
  ...
  renderFields () {
    let { main, modifiers, filter } = atom.config.get(PACKAGE_NAME)
  ...
  render () {
    ...
    let overrides = atom.config.get(`${PACKAGE_NAME}.override`) || {}
    return (
      <section className='syntax-preview'>
        ...
        <aside className='config padded'>
          ...
          <section className='section overrides-section'>
            {Object.keys(overrides).map(override => (
              <div className='color inverse primary' id={`override-${override}`} />
            ))}
            <div className='color inverse primary variant sub circle icon icon-plus'
            style={{textAlign: 'center', lineHeight: "var(--size)", fontSize: '2em', color: "white" }}/> Add override
          </section>
