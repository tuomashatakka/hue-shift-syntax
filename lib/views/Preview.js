...
export default class PreviewView extends Component {
  ...
  render () {
    ...
    // let editor = this.editor ? this.editor.getElement() : null
    return (
      <section className='syntax-preview'>

        <header className='main-header'>
          <h3><span className='icon icon-paintcan' /> Syntax Color Configuration</h3>
        </header>

        {/*<article
          className='editor'
          ref={ref => ref && editor ? ref.appendChild(editor) : ref}>
        ...
        <aside className='config'>
          ...
          <section className='section presets-section'>
            <PresetList
              ...
              onChange={(name) => this.updateSections()} />
          </section>
